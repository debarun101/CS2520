...
void neighbor_cease(string, unsigned short);
void *Print(void* args)
{
	while(1)
	{
		...
		cout<<".. Cease Neighbors (Press 'CS')\n";
		cout<<".. Change Probability  of Introducing Error(Press Pe)\n";
		cout<<".. Change Probability Of Dropping Packets(Press Pc)\n\n";
		getline(cin, choice);
		...
		//CHANGE....
		else if (choice == "CS" || choice == "cs")
		{
			string IPadd,edgePort ;
			printmyNeighbors();
			cout<<"\n\nChoose The Router TO Cease Relation..\n";
			...
			neighbor_cease(IPadd, edPort);
		}
		else if(choice == "PC" ||choice == "Pc" || choice == "pc" || choice == "pC")
		{
			cout<<"\nEnter Pc (between 0 and 100)";
			string pc;
			getline(cin, pc);
			double perror = (double)strtoul(pc.c_str(), NULL, 0);
			Pc = perror;
		}
		else if(choice == "PE" ||choice == "Pe" || choice == "pe" || choice == "pE")
		{
			cout<<"\nEnter Pe (between 0 and 100)";
			string pc;
			getline(cin, pc);
			double perror = (double)strtoul(pc.c_str(), NULL, 0);
			Pe = perror;
...
//Function To Cease Neighbor
//CHANGE...
void neighbor_cease(string IP, unsigned short port)
{
	try
	{
		if(Debug == 1)
		cout<<"\ncease Neighbor IP:"<<IP<<", Cease Neighbor port:"<<port<<"\n";

		ClientSocket sock(IP, port);
		//Message Sent To Edge Router Before Transferring A File
		string Msg = "Cease_Neighbors:Port_"+Gport+"*";
		...
		if(requestType == "Cease_Confirm")
		{
			//Remove The Entry From the Neighbor Table
...
void dijkstra(int **graph, int src, int count)
{
	...
	int *nextHop = new int[count+count]; // output array, nextHop[i] will hold the next hop that src to i;
	
	int **index = new int*[count+count];
	for(int i = 0; i<count; i++)
		index[i] = new int[count+count];

	int **index1 = new int*[count+count];
	for(int i = 0; i<count; i++)
		index1[i] = new int[count+count];
	
	int *index2 = new int[count+count];
	int hop;
	int source;
		...
		//{
			...
			else
			{
				...
				for(LinkData::iterator it = Database.begin(); it != Database.end(); it++)
				{
					...
					if(strcmp(it->second.sourceIP.c_str(),destinationIP.c_str()) == 0 && it->second.sourcePort == destUport)
					{
						if(Debug == 1)
						cout<<"\nLSD Match Found\n";
						Database.erase(it);
					}
					if(strcmp(it->second.destinationIP.c_str(),destinationIP.c_str()) == 0 && it->second.destinationPort == destUport)
					{
						if(Debug == 1)
						cout<<"\nLSD Match Found\n";
						Database.erase(it);
				...
				for(int i =0; i< vect.size();i++)
				{
					...
					if(strcmp(vect[i].sourceIP.c_str(),destinationIP.c_str())==0 && vect[i].sourcePort == destUport)
					{
						if(Debug == 1)
						cout<<"\nIncoming List Match Found\n";
						vect.erase (vect.begin()+i);
					}
	...
	while(1)
	{
		...
		for(int i =0; i< vect.size();i++)
		{
			...
			try
			{
				...
				{
					//cout<<"\nLSA about to flood:";
					//cout<<"\n LSA-> port"<< neighbor_table[j].port<<","<<neighbor_table[j].IP<<",end:"<<neighbor_table[j].endSystem<<",AckN:"<<neighbor_table[i].AckN<<"\n";
					...
					if( neighbor_table[j].port != 0 && strcmp(neighbor_table[j].IP.c_str(),"0.0.0.0") !=0 && neighbor_table[j].endSystem == 0 )
					{
						...
						//cout<<"LSA Flooded to Neighbor :"<<neighbor_table[j].IP<<","<<neighbor_table[j].port<<"\n";
						//cout<<"\nLSA packet flooded:"<<vect[i].LSAPacket<<"\n";
...
void eraseList(string IP, unsigned short port, string LSAPacket)
{
	...
	//Don't add the its own LSA from others
...
{
	//if(strcmp(sourceIP.c_str(), GlobalAddress.c_str())!=0 && GlobalPort != sourcePort)
	//{ 
...
void alive(string IP, unsigned short port)
{
	...
	if(flag == 1)
	{
		...
		try
		{
			for(int mn = 0; mn < key; mn++)
			{
				if(neighbor_table[mn].AckN == 1 && neighbor_table[mn].endSystem == 0)
				{

					string neighborIP = neighbor_table[mn].IP;
					unsigned short neighborPort = neighbor_table[mn].port;
					ClientSocket sock(neighborIP, neighborPort);

					cout<<"\nNeighbor IP: "<<neighborIP<<", Neighbor Port"<<neighborPort<<"\n";
			
					char buffer[32];
					snprintf(buffer, 32, "%hu",port);
					string temp(buffer);

					string str = "DeadNeighbor:myprt_"+Gport+":IP_"+IP+":Port_"+temp+"*";
...
void* neighbor_acq(void *args)
{
	...
	try
	{
		...
		{
			pthread_mutex_lock(&count_mutex);
			if(neighbor_table[current].AckN != 1)
			{
				if(Debug == 1)
				cout<<"\nentered if\n";
			...
			while ((recvMsgSize = sock->receiveData(echoBuffer,RECEIVE_BUFFER_SIZE)) > 0) 
			{
				...
				if(requestType == "BeNeighbors")
				{
					...
					for(int i = 0;i < blkey; i++)
					{
						...
						if(strcmp(callerAddress.c_str(), bl[i].IP.c_str()) == 0 )//&& u_port ==  bl[i].port)
						{
							flag = 1;
					...
					//Check If Not In Neighbor Table Already
					int flag1 = 0;
					for(int mn = 0; mn < key; mn++)
					{
						if(strcmp(neighbor_table[mn].IP.c_str(), callerAddress.c_str()) == 0 && neighbor_table[mn].port ==u_port)
						{
							flag1 =1;
						}	
				...
				else if(requestType == "Cease_Neighbors")
				{
					char * response = "Cease_Confirm:";
					int len = strlen(response);
					sock->sendData(response, len);

					//Remove This EX Neighbor From ALL Places
					int index2 = receivedData.find("Port_")+5;
					int length = receivedData.find("*") - index2;
					string port = receivedData.substr(index2, length);
					...
					const char* const_port = port.c_str();
					unsigned short u_port = (unsigned short) strtoul(const_port, NULL, 0);
					//cout<<"Receievd Cease port:"<<u_port;
				
					//Remove The Entry From the Neighbor Table
					pthread_mutex_lock(&count_mutex);
					for(int mn = 0; mn < key; mn++)
					{
						if(neighbor_table[mn].port == u_port && strcmp(neighbor_table[mn].IP.c_str(), callerAddress.c_str())==0)
						{
							neighbor_table[mn].port =0;
							neighbor_table[mn].IP = "0.0.0.0";
							neighbor_table[mn].SendN = 0;
					...
					for(LinkData::iterator it = Database.begin(); it != Database.end(); it++)
					{
						...
						if(strcmp(it->second.sourceIP.c_str(),callerAddress.c_str()) == 0 && it->second.sourcePort == u_port)
						{
					...
					for(int i =0; i< vect.size();i++)
					{
						...
						if(strcmp(vect[i].sourceIP.c_str(),callerAddress.c_str())==0 && vect[i].sourcePort == u_port)
						{
							vect.erase (vect.begin()+i);
						}
					...
					for(Route_Table::iterator it = Rtable.begin(); it != Rtable.end(); it++)
					{
						if(Debug == 1){
						cout<<"\n TO DELETE CONTENT FROM ROUTING TABLE\n";
						cout<<"\nDead Address: "<<callerAddress<<", Dead Port:"<<u_port<<"\n";
						cout<<"\nRtable Address: "<<it->second.destinationIP<<", RTable Port: "<<it->second.destinationPort<<"\n";
						}
						if(strcmp(it->second.destinationIP.c_str(),callerAddress.c_str())==0 && it->second.destinationPort == u_port)
						{
							if(Debug == 1)
							cout<<"\nMatch Founfd\n";
							Rtable.erase(it);
				...
				{
					cout<<"\n\nRequest Received: DEADNEIGHBOR\n";
					//"DeadNeighbor:myprt_"+Gport+":IP_"+IP+":Port_"+temp+"*";;
					/*int srcindex = receivedData.find("myprt_")+6;
					int length = receivedData.find(":IP_") - srcindex;
					string srcport = receivedData.substr(srcindex, length);
					cout<<"\nSource port:"<<srcport<<"\n";
					unsigned short sp = (unsigned short) strtoul(srcport.c_str(), NULL, 0);//comment ends here

					int IPindex = receivedData.find("IP_")+3;
					int length = receivedData.find(":Port_") - IPindex;
					string sourceIP = receivedData.substr(IPindex, length);
					cout<<"\nDead IP: "<<sourceIP;

					int portindex = receivedData.find("Port_") + 5;
					...
					//string str = "DeadNeighbor:myprt_"+Gport+":IP_"+sourceIP+":Port_"+port+"*";
					
					
					pthread_mutex_lock(&count_mutex);
					for(int mn = 0; mn < key; mn++)
					{
						if(neighbor_table[mn].port == sourcePort && strcmp(neighbor_table[mn].IP.c_str(), sourceIP.c_str())==0)
						{
							cout<<"\nNeighbor Table Match Found\n";
							neighbor_table[mn].port =0;
							neighbor_table[mn].IP = "0.0.0.0";
							neighbor_table[mn].SendN = 0;
							neighbor_table[mn].AckN = 0;
					...
					for(LinkData::iterator it = Database.begin(); it != Database.end(); it++)
					{
						...
						if(strcmp(it->second.sourceIP.c_str(),sourceIP.c_str()) == 0 && it->second.sourcePort == sourcePort)
						{
							cout<<"\nLSD Match Found\n";
							Database.erase(it);
						}
						if(strcmp(it->second.destinationIP.c_str(),sourceIP.c_str()) == 0 && it->second.destinationPort == sourcePort)
						{
							cout<<"\nLSD Match Found\n";
							Database.erase(it);
						}
					...
					for(int i =0; i< vect.size();i++)
					{
						...
						if(strcmp(vect[i].sourceIP.c_str(),sourceIP.c_str())==0 && vect[i].sourcePort == sourcePort)
						{
							cout<<"\nIncoming List Match Found\n";
							vect.erase (vect.begin()+i);
						}
					...
					{
						cout<<"\n TO DELETE CONTENT FROM ROUTING TABLE\n";
						cout<<"\nDead Address: "<<sourceIP<<", Dead Port:"<<sourcePort<<"\n";
						cout<<"\nRtable Address: "<<it->second.destinationIP<<", RTable Port: "<<it->second.destinationPort<<"\n";
						if(strcmp(it->second.destinationIP.c_str(),sourceIP.c_str())==0 && it->second.destinationPort == sourcePort)
						{
							cout<<"\nRouting Match Founfd\n";
							Rtable.erase(it);
							break;
					...
					pthread_mutex_unlock(&RT_mutex);
					/*cout<<"\n====SENDING NEIGHBOR REQUEST TO OTHER ROUTERS====\n";
					...
					try
					{
						for(int mn = 0; mn < key; mn++)
						{
							cout<<"\nSEND NEIGHBOR IP:"<<neighbor_table[mn].IP<<",Send Neighbor Port:"<<neighbor_table[mn].port;
							//ELEMENTS .. Add && neighbor_table[mn].IP!= callerAddress
							if(neighbor_table[mn].AckN == 1 && neighbor_table[mn].endSystem == 0)
							{
								cout<<"\nSEND NEIGHBOR IP:"<<neighbor_table[mn].IP<<",Send Neighbor Port:"<<neighbor_table[mn].port;

								if(strcmp(neighbor_table[mn].IP.c_str(),callerAddress.c_str())!=0 && sp!= neighbor_table[mn].port) 
								{
									cout<<"\nYes, NOTIFICATION OF DEAD SENT:\n";
									string neighborIP = neighbor_table[mn].IP;
									unsigned short neighborPort = neighbor_table[mn].port;
									ClientSocket sock(neighborIP, neighborPort);
			
									cout<<"\nSending TO Neighbor IP: "<<neighborIP<<", Neighbor Port"<<neighborPort<<"\n";
									//string str = receivedData.substr(0,receivedData.find("*"))+"*" ;
	...
	try{
		...
		for (int i=1; recv[i]!='$'; ++i)
		{
			...
			else
			{
				...
				{
					//check if the requesting neighbor is blacklisted
					if(strcmp(IP.c_str(), bl[i].IP.c_str()) == 0 )//&& u_port ==  bl[i].port)
					{
						flag = 1;
...
void configure(string daddress, unsigned short dport)
{
	...
	while(1)
	{
		...
		//CHANGED...max age added.
		else if(input == "age" ||input == "AGE" || input == "Age")
		{
			cout<<"\nChange Max Age Of The Router\n";
			string age;
			getline(cin,age);
			unsigned int perror = (unsigned int)strtoul(age.c_str(), NULL, 0);
			MaxAge = perror;
		}
