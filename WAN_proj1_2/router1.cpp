...
void* alive_call(void* args)
{
	...
	cout <<"\nentered alive_call actual\n";
		...
		while(1)
		{
			...
			for(int i = 0; i < key; i++)
			{
				//Drop packet with probability Pc. Generating Random numbers to determine this. 
				int v2 = rand()%100 +1;
				if(v2 > Pc)
				{
					if(Debug == 1)
	...
	for(int i = 0; i< 4; i++)
	{
		try
		{
			...
			while(CRCflag >0)
			{
				...
				if(Debug == 1)
				cout<<"\nAlive message checksum:"<<checksum<<"\n";
				...
				str = "";
				str = "Alive:0#CRC_"+check+"&";
			
			
				//Generating a Random Number between 1 and 100 and determining if an error is to be introduced in a packet or Not
				int v1 = rand()%100+1;
				...
				{
					if(Debug == 1)
					cout<<"\nCRC error.. Resending Alive Message\n";
	...
	try
	{
		...
		while(1)
		{
			...
			while ((recvMsgSize = sock->receiveData(echoBuffer,RECEIVE_BUFFER_SIZE)) > 0) 
			{
				...
				{
					int index2 = receivedData.find("CRC_") + 4; 
					int length = receivedData.find("&") - index2;
					string scrc = receivedData.substr(index2, length);
					if(Debug == 1)
					cout<<"\nreceived crc:"<<scrc<<"\n";
				unsigned short u_crc = (unsigned short) strtoul(scrc.c_str(), NULL, 0);	
					if(Debug == 1)
					cout<<"\nshort receive crc:"<<u_crc;
					
					string actualMsg = receivedData.substr(0,receivedData.find("#"));				
					unsigned short calc_CRC = crc16((char*)actualMsg.c_str(),strlen(actualMsg.c_str()));
					if(Debug == 1)
...
//Get The NameServer IP and Port 
void readNameConfig()
{
